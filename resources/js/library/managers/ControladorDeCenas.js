export class ControladorDeCenas {
    ...
    mudarCena(index) {
        if (index >= 0 && index < this.cenas.length) {
            ...
            // Para apenas efeitos sonoros, preservando música de fundo
            if (this.game.sound) {
                //console.log('Parando efeitos sonoros...');
                
                // Para apenas efeitos sonoros, não música de fundo
                this.game.sound.sounds.forEach(sound => {
                    if (sound && sound.key !== 'musica') { // Preserva música de fundo
                        sound.stop();
                        sound.destroy();
                    }
                });
